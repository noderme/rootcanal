...
function rankBand(rank: number): string {
  ...
  if (rank <= 20)
    return "Usually appears outside the main patient decision range";
      ...
      >
        ...
        >
          ...
          {mode === "both" && (
            ...
            >
              ...
              <div style={{ fontSize: 13, color: "#6B7B78", marginBottom: 24 }}>
                For clinics that want to improve local ranking visibility and
                attract more patient enquiries.
              </div>
              <div
                style={{
                  flex: 1,
          ...
          >
            ...
            <div style={{ fontSize: 13, color: "#6B7B78", marginBottom: 24 }}>
              For clinics aiming to consistently appear ahead of nearby
        ...
        >
          <div style={{ fontSize: 12, color: "#4A5A57", letterSpacing: 0.3 }}>
            Built specifically for dental clinics improving their Google
            visibility.
          </div>
          <div
            style={{
      ...
      const map = new G.Map(mapRef.current, {
        ...
        styles: [
          ...
          { elementType: "labels.text.fill", stylers: [{ color: "#8EC5B0" }] },
          {
            ...
            stylers: [{ color: "#0D1714" }],
          },
          {
            featureType: "road",
            elementType: "geometry",
            stylers: [{ color: "#2A3A35" }],
          },
          {
            featureType: "road",
            elementType: "labels.text.fill",
            stylers: [{ color: "#6B9E8A" }],
          },
          {
            featureType: "water",
            elementType: "geometry",
            stylers: [{ color: "#0D2118" }],
          },
...
function DashboardContent() {
  ...
  const [authState, setAuthState] = useState<
    "checking" | "authenticated" | "enter-email" | "enter-otp"
  >("checking");
    ...
    try {
      const res = await fetch("/api/request-review", {
        ...
        body: JSON.stringify({
          contact: val,
          type,
          clinicName: data.clinicName || nameParam,
          clinicUrl: data.url,
  ...
  useEffect(() => {
    ...
    const rawUserRank =
      data.userRank ??
      (data.competitors.length > 0 ? data.competitors.length + 1 : undefined);
    ...
    const storedStillValid =
      stored != null && stableAhead.some((c) => c.name === stored);
  ...
  useEffect(() => {
    setReviewStripDismissed(
      sessionStorage.getItem("rc_review_strip_dismissed") === "1",
    );
  }, []);
    ...
    const init = async () => {
      ...
      if (url) {
        const res = await fetch(
          `/api/lookup-user?url=${encodeURIComponent(url)}`,
        );
        const data = await res.json();
        if (data.found) {
  ...
  const sendOtp = async () => {
    ...
    if (error) {
      setAuthError("Failed to send code. Try again.");
      return;
    }
    setAuthState("enter-otp");
  };
  ...
  // Review velocity — fetch once when clinic URL is known
  useEffect(() => {
    if (!data?.url) return;
    const clinicUrl = data.url;
    (async () => {
      ...
      const { data: recent } = await supabase
        ...
        .limit(1)
        .single();
      if (!recent || recent.review_count == null) return;
      ...
      const { data: older } = await supabase
        ...
        .limit(1)
        .single();
      if (!older || older.review_count == null) return;

      const gained = recent.review_count - older.review_count;
      const diffDays = Math.round(
        (recentDate.getTime() - new Date(older.snapshot_date).getTime()) /
          (1000 * 60 * 60 * 24),
      );
      setReviewVelocity({ gained, days: diffDays });
    })();
  ...
  const loadingSteps = [
    ...
    {
      label:
        "Evaluating factors that influence patient discovery and booking decisions…",
    },
    {
    ...
    {
      label:
        "Understanding what influences patients when choosing between nearby clinics…",
    },
    {
      label:
        "Detecting patterns in patient feedback that may influence trust and referrals…",
    },
    {
      label:
        "Comparing how easily patients can discover your clinic versus nearby competitors…",
    },
        ...
        >
          ...
          {authState === "enter-email" ? (
            <>
              ...
              >
                Enter your email to access your report
              </p>
              <input
                type="email"
                value={authEmail}
              ...
              {authError && (
                <div
                  style={{ color: "#E74C3C", fontSize: 13, marginBottom: 12 }}
                >
                  {authError}
                </div>
              )}
              <button
                ...
                style={{
                  ...
                  border: "none",
                  cursor: "pointer",
                  opacity: authLoading || !authEmail.includes("@") ? 0.5 : 1,
                }}
              >
            ...
            <>
              ...
              <p style={{ color: "#F7F3ED", fontSize: 14, marginBottom: 28 }}>
                {authEmailDisplay || authEmail}
              </p>
              <div
                style={{
                  display: "flex",
                ...
                {Array.from({ length: 6 }).map((_, i) => (
                  <input
                    ...
                    style={{
                      ...
                      height: 52,
                      borderRadius: 10,
                      border: `1px solid ${authOtp[i] ? "#1ABC9C" : "#2A3330"}`,
                      background: "#0D0F0E",
                      color: "#F7F3ED",
                      fontSize: authOtp[i] ? 22 : 18,
                      textAlign: "center",
                      outline: "none",
                      fontFamily: "'DM Sans', sans-serif",
                    }}
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  cursor: "pointer",
                  opacity: authLoading || authOtp.length < 6 ? 0.5 : 1,
                }}
              >
  ...
  if (loading)
    return (
      ...
      >
            ...
            {loadingSteps.map((step, i) => (
              <div
                ...
                style={{
                  ...
                  animation:
                    i <= loadingStep ? `fadeSlideUp 0.3s ease both` : "none",
                }}
              >
                <div
                  style={{
                    ...
                    alignItems: "center",
                    justifyContent: "center",
                    background: i < loadingStep ? "#1ABC9C" : "transparent",
                    border:
                      i < loadingStep
                        ? "none"
                        : `2px solid ${i === loadingStep ? "#1ABC9C" : "#2A3330"}`,
                  }}
                >
                  {i < loadingStep ? (
        ...
        >
          This analysis typically takes a few seconds and helps identify
  ...
  // Full Health tab access: trial users, pro, and growth subscribers
  const hasHealthAccess = isTrial || isPro || isGrowth;
  ...
  const competitorStabilityScore = (() => {
    const withData = data.competitors.filter(
      (c) => c.appearances !== undefined,
    );
  ...
  const volatilityNote = (() => {
    ...
    if (competitorStabilityScore < 0.6)
      return {
        text: "Local competitor visibility fluctuated recently.",
        dot: "#4A5A58",
      };
    return {
      text: "Competitor visibility has been relatively consistent.",
      dot: "#2A4A40",
    };
  })();

  // competitionIntensity — derived from smoothed ahead count, not raw snapshot
  const smoothedAheadCount =
    displayRank != null ? Math.max(0, displayRank - 1) : null;
  const competitionIntensity = (() => {
    if (smoothedAheadCount == null) return null;
    if (smoothedAheadCount <= 3)
      return {
        label: "Low Competition",
        bg: "rgba(46,204,113,0.1)",
        color: "#2ECC71",
      };
    if (smoothedAheadCount <= 8)
      return {
        label: "Moderate Competition",
        bg: "rgba(52,152,219,0.1)",
        color: "#5DADE2",
      };
    if (smoothedAheadCount <= 15)
      return {
    ...
    return {
      ...
      bg: "rgba(180,90,70,0.1)",
      color: "#C07860",
    };
  })();

  const reviewRank = (() => {
    if (data.userReviewCount == null || data.competitors.length === 0)
      return undefined;
    const allReviews = [
      ...data.competitors.map((c) => c.reviews),
  ...
  // Top competitor by review count — used for review gap display
  const topCompByReviews = data.competitors.length > 0
    ? [...data.competitors].sort((a, b) => (b.reviews || 0) - (a.reviews || 0))[0]
    : null;
  const topCompReviews = topCompByReviews?.reviews || 0;
  const myReviewCount = reviews?.total ?? data.userReviewCount ?? 0;
  const reviewGapVsTop = Math.max(0, topCompReviews - myReviewCount);
  ...
  if (showTeaser) {
    ...
    const reviewGapVsTop =
      topComp != null
        ? Math.max(0, (topComp.reviews || 0) - userReviewCount)
        : null;
        ...
        <div className="teaser-card" style={{ width: "100%", maxWidth: 520 }}>
          ...
          >
            ...
            <div
              style={{
                background: "#151918",
                border: `1px solid ${tRank == null || tRank > 3 ? "rgba(231,76,60,0.3)" : "rgba(46,204,113,0.3)"}`,
                borderRadius: 14,
                padding: "18px 22px",
                display: "flex",
                alignItems: "center",
            ...
            >
              ...
              >
                {tRank != null ? (
                  <>
                    <div
                      style={{
                        fontSize: 15,
                        fontWeight: 700,
                        color: tRank <= 3 ? "#1ABC9C" : "#D4A843",
                        lineHeight: 1.3,
                      }}
                    >
                      {rankShortLabel(tRank)}
                    </div>
                    <div
                  ...
                  <div
                    style={{ fontSize: 24, fontWeight: 700, color: "#6B7B78" }}
                  >
                    —
                  </div>
                )}
            ...
            >
              <div>
                ...
                >
                  Patients Lost to Competitors / Month
                </div>
                <div
                  style={{
                    fontSize: 11,
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <a
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.color = "#6B7B78";
              e.currentTarget.style.borderColor = "#3A4A47";
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.color = "#4A5A57";
              e.currentTarget.style.borderColor = "#2A3330";
            }}
          >
            ← Home
          </a>
          ...
          <div style={{ padding: "16px 12px 0" }}>
            {isGrowth ? (
              ...
              >
                ...
                >
                  All features unlocked. Your clinic is on the path to #1.
                </div>
              </div>
            ) : (
              <div
        ...
        >
          <div style={{ marginBottom: 24 }}>
                ...
                (nameParam
                  ...
                  : url
                    ? url
                        ...
                        .split("/")[0]
                        .replace(
                          /\.(com|net|org|io|us|dental|care|health)$/,
                          "",
                        )
                        .replace(/[-_.]/g, " ")
            ...
            >
              ...
              >
                📡 Powered by Google Maps, Yelp, Google PageSpeed & live web
                data
              </span>
            </div>
          </div>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: compactHero
                        ...
                        : "0 4px 24px rgba(231,76,60,0.1), 0 2px 10px rgba(0,0,0,0.4)",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "space-between",
                      gap: 24,
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 220 }}>
                      ...
                      >
                        {myReviewCount > 0 && topCompReviews > 0 ? (
                          <>
                            You have <span style={{ color: "#F0EBE3", fontWeight: 700 }}>{myReviewCount} Google reviews</span>.
                            {" "}The top clinic nearby has <span style={{ color: "#F0EBE3", fontWeight: 700 }}>{topCompReviews}</span>
                            {reviewGapVsTop > 0 ? <> — a gap of <span style={{ color: "#F0A500", fontWeight: 700 }}>{reviewGapVsTop} reviews</span></> : " — you're ahead on reviews"}.
                          </>
                        ) : (
                          <>Clinics with more reviews typically receive more patient enquiries. Growing your review count is the highest-impact action.</>
                        )}
                        {displayRank != null && (
                          <div style={{ fontSize: 11, color: "rgba(240,235,227,0.3)", marginTop: 6 }}>
                            Currently appearing {rankRangeText(displayRank, data.rankRangeLow, data.rankRangeHigh)} in local results
                          </div>
                        )}
                      </div>
                      <div
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {refreshError && (
                <span style={{ fontSize: 11, color: "#6B7B78" }}>
                  {refreshError}
                </span>
              )}
              <button
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      fontSize: 28,
                      fontWeight: 900,
                      color: reviewGapVsTop === 0 ? "#1ABC9C" : "#F0EBE3",
                      lineHeight: 1,
                      marginTop: 4,
                    }}
                  >
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        fontSize: 28,
                        fontWeight: 900,
                        color: reviewRank <= 3 ? "#1ABC9C" : "#E74C3C",
                        lineHeight: 1,
                      }}
                    >
                      #{reviewRank}
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        fontSize: 28,
                        fontWeight: 900,
                        color: data.yelpRank <= 3 ? "#1ABC9C" : "#E74C3C",
                        lineHeight: 1,
                      }}
                    >
                      #{data.yelpRank}
              ...
              {(
                [
                  {
                    ...
                    label: "Website Speed",
                    sublabel:
                      "slow sites can meaningfully reduce patient conversions",
                    color:
                  ...
                  },
                  {
                    ...
                    sublabel:
                      "poor findability costs you new patient bookings daily",
                    color:
                      data.seoScore >= 70
                        ? "#2ECC71"
                        : data.seoScore >= 40
                  ...
                  },
                  {
                    ...
                    sublabel:
                      "hard-to-use sites reduce form completions and enquiries",
                    color:
                      data.accessibilityScore >= 70 ? "#2ECC71" : "#F0A500",
                  },
                ] as {
                  icon: string;
                  value: string | number;
                ...
                <div
                  ...
                  style={{
                    ...
                    gap: 12,
                    borderRight:
                      i < arr.length - 1 ? "1px solid #2A3330" : "none",
                  }}
                >
                  ...
                  <div>
                    ...
                    <div
                      style={{ fontSize: 10, color: "#4A5A57", marginTop: 1 }}
                    >
                      {m.sublabel}
                    </div>
                  </div>
          ...
          >
            Based on real local search results and recent patient review
            activity.
          </div>
          ...
          {activeTab === "competitors" && (
            <>
              {(() => {
                ...
                const PAGE_SIZE = 10;
                const allSorted = [...data.competitors].sort(
                  (a, b) => a.googleRank - b.googleRank,
                );

                // Build display rows with user row injected at userRank position
                type Row =
                  | { isUser: true; rank: number }
                  | { isUser: false; comp: (typeof allSorted)[0] };
                const rows: Row[] = [];
                ...
                const pageRows = rows.slice(
                  competitorPage * PAGE_SIZE,
                  (competitorPage + 1) * PAGE_SIZE,
                );
                  ...
                  >
                    ...
                    >
                      <div>
                        ...
                        >
                          Rankings are measured within a 5km radius of your clinic. Google also weighs proximity and profile completeness — which is why some newer listings may rank above clinics with more reviews.
                        </div>
                      </div>
                    ...
                    {(() => {
                      ...
                      const stableAheadCount = data.competitors.filter(
                        (c) =>
                          c.googleRank < (userRank ?? 999) &&
                          (c.appearances ?? 0) >= 1,
                      ).length;
                      const hasStableHistory = data.competitors.some(
                        (c) => c.appearances !== undefined,
                      );
                    ...
                    {/* Column headers */}
                    <div
                      className="rc-comp-table-header"
                      style={{
                        display: "grid",
                        gridTemplateColumns: "48px 1fr 80px 80px 90px",
                        padding: "10px 16px",
                        borderBottom: "1px solid #1A2220",
                        fontSize: 11,
                        color: "#4A5A58",
                        ...
                        >
                          ...
                          <div>
                            ...
                            >
                              ...
                              {(!comp.reviews || comp.reviews === 0 || !comp.rating || comp.rating === 0) && (
                                <span
                                  style={{
                                    fontSize: 10,
                                    fontWeight: 500,
                                    padding: "1px 6px",
                                    borderRadius: 10,
                                    background: "rgba(107,123,120,0.10)",
                                    color: "#5A6B68",
                                    fontFamily: "'DM Mono', monospace",
                                    whiteSpace: "nowrap",
                                    border: "1px solid rgba(107,123,120,0.18)",
                                  }}
                                >
                                  New listing
                                </span>
                    ...
                    {totalPages > 1 && (
                      <div
                        style={{
                          ...
                          justifyContent: "space-between",
                          padding: "12px 16px",
                          borderTop: "1px solid #1A2220",
                        }}
                      >
                        <button
                          onClick={() =>
                            setCompetitorPage((p) => Math.max(0, p - 1))
                          }
                          disabled={competitorPage === 0}
                          style={{
                            background: "none",
                            border: "1px solid #2A3330",
                            color: competitorPage === 0 ? "#2A3330" : "#C8BFB6",
                            borderRadius: 8,
                            padding: "6px 14px",
                            fontSize: 12,
                            cursor:
                              competitorPage === 0 ? "default" : "pointer",
                          }}
                        >
                          ← Prev
                        </button>
                        <span style={{ fontSize: 12, color: "#6B7B78" }}>
              ...
              {data.competitorCountOutlier && (
                ...
                >
                  ...
                  />
                  <span>
                    Competitor count appears lower than usual — results will
                    stabilise as more data is collected.
                  </span>
                </div>
              )}
              <div
              ...
              >
                ...
                <span>
                  {volatilityNote?.text ??
                    "Nearby competing clinics can vary across neighbourhood searches and devices."}{" "}
          ...
          {activeTab === "roadmap" && (
            ...
            >
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#6B7B78" }}>
                    Scores are out of 100 — combining your Google reviews, SEO,
                    and website quality.
                  </div>
                  {/* Micro-progress bar */}
                  <div
                  ...
                  >
                    ...
                    >
                      Step 1 of your visibility journey
                    </span>
                  </div>
                </div>
                <span
              ...
              {(() => {
                ...
                if (userRank == null) {
                  return (
                    <div style={{ textAlign: "center", padding: "32px 20px" }}>
                      ...
                      >
                        Your practice isn&apos;t visible in the top 60 Google
                        results
                      </div>
                      <div
                        style={{
                      ...
                      >
                        Clinics outside the top 60 lose an estimated 25–40
                        patients per month to ranked competitors. More Google
                        reviews is the single fastest way to start appearing —
                ...
                const aheadCountLow =
                  data.rankRangeLow != null
                    ? Math.max(0, data.rankRangeLow - 1)
                    : null;
                const aheadCountHigh =
                  data.rankRangeHigh != null
                    ? Math.max(0, data.rankRangeHigh - 1)
                    : null;
                const competitorContextLabel = (() => {
                  if (
                    aheadCountLow != null &&
                    aheadCountHigh != null &&
                    aheadCountLow !== aheadCountHigh
                  ) {
                    ...
                    >
                      ...
                      >
                        ...
                        <div>
                          ...
                          >
                            These practices are closing in. Clinics that stop
                            collecting reviews drop in rank within weeks —
                            losing bookings to the clinics that didn&apos;t.
                          </div>
                        </div>
                      </div>
                        ...
                        const urgencyColor =
                          urgency === "high"
                            ...
                            : urgency === "medium"
                              ...
                              : "#6B7B78";
                        const compName =
                          (comp.name || "Competitor").length > 28
                          ...
                          >
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              {!isLocked ? (
                                ...
                                >
                                  Only {gap} review{gap === 1 ? "" : "s"} behind
                                  you — if they overtake your rank, they capture
                                  your patients. Estimated{" "}
                                  <span
                                    style={{
                                      color: urgencyColor,
                                      fontWeight: 600,
                                  ...
                                  until they could outrank you.
                                  {urgency === "high" ? " Act now." : ""}
                                </div>
                              ) : (
                                <div style={{ fontSize: 12, color: "#6B7B78" }}>
                                  🔒 Unlock to see how close this clinic is to
                                  stealing your rank
                                </div>
                              )}
                            </div>
                            <div style={{ textAlign: "right", flexShrink: 0 }}>
                              <div
                                style={{
                                  ...
                                  fontSize: 16,
                                  fontWeight: 700,
                                  color: isLocked ? "#6B7B78" : urgencyColor,
                                }}
                              >
                                Pos. {comp.googleRank}
                              </div>
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 12, color: "#6B7B78" }}>
                            Every review widens your lead — clinics with more
                            reviews consistently attract more appointment
                            requests and hold rank longer.
                          </div>
                        </div>
                        <div
                          style={{
                ...
                const pinnedAheadComps = (() => {
                  ...
                  const idx = aheadComps.findIndex(
                    (c) => c.name === roadmapTargetName,
                  );
                ...
                const visibleSteps = freeUser
                  ...
                  : stepsToShow;
                const hiddenCount = freeUser
                  ? Math.max(0, stepsToShow.length - 3)
                  : 0;
                return (
                  <div
                    style={{
                  ...
                  >
                    ...
                    {freeUser && stepsToShow.length > 0 && (
                      ...
                      >
                        ...
                        >
                          Each step you complete moves more local patients
                          toward your clinic.{" "}
                          <span style={{ color: "#6B7B78" }}>
                            {competitorContextLabel}
                          </span>
                        </div>
                      </div>
                    )}
                              ...
                              {i === 0 && roadmapTargetChanged && (
                                <span
                                  style={{
                                    ...
                                    padding: "2px 8px",
                                    borderRadius: 4,
                                    background: "rgba(107,123,120,0.1)",
                                    color: "#6B7B78",
                                    fontFamily: "'DM Mono', monospace",
                                  }}
                                >
                    ...
                    {hiddenCount > 0 && (
                      ...
                      >
                        ...
                        <div
                          style={{ flex: 1, fontSize: 12, color: "#6B7B78" }}
                        >
                          {aheadCountLow != null &&
                          aheadCountHigh != null &&
                          aheadCountLow !== aheadCountHigh
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          {isGrowth || isTrial || isPro ? (
                            ...
                            <div style={{ fontSize: 12, color: "#6B7B78" }}>
                              🔒 Unlock your full step-by-step patient growth
                              plan
                            </div>
                          )}
                        </div>
            ...
            >
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  {reviews?.reviewSources && (
                    ...
                    >
                      {reviews.reviewSources.google}G +{" "}
                      {reviews.reviewSources.yelp}Y
                    </span>
                  )}
                  <span
                    style={{
              ...
              {reviewsLoading ? (
                ...
                >
                  ...
                  >
                    With Pro, we automatically ask your patients for reviews
                    after every visit — so your clinic builds trust while you
                    focus on care.
                  </div>
                  {!isGrowth && (
                    <button
                            ...
                            {reviewVelocity != null && (
                              <div
                                style={{
                                  fontSize: 11,
                                  marginTop: 4,
                                  color: reviewVelocity.gained > 0 ? "#2ECC71" : "#6B7B78",
                                  fontFamily: "'DM Mono', monospace",
                                }}
                              >
                                {reviewVelocity.gained > 0
                                  ? `+${reviewVelocity.gained} in last ${reviewVelocity.days}d`
                                  : `0 new in last ${reviewVelocity.days}d`}
                              </div>
                            )}
                          </div>
                          <div style={{ fontSize: 20, color: "#2A3330" }}>
                            vs
                          </div>
                          <div style={{ textAlign: "center" }}>
                            <div
          ...
          {activeTab === "health" && (
            ...
            >
              ...
              {(() => {
                const signals = [
                  { label: "Photos uploaded", pass: (data.gbpPhotoCount ?? 0) > 0 },
                  { label: "Business hours set", pass: !!data.gbpHasHours },
                  { label: "Website linked", pass: !!data.gbpHasWebsite },
                  { label: "Phone number set", pass: !!data.gbpHasPhone },
                  { label: "15+ Google reviews", pass: (data.userReviewCount ?? 0) >= 15 },
                ];
                  ...
                  >
                    ...
                    >
                      <div>
                        ...
                        >
                          These signals directly influence your Google Maps ranking
                        </div>
                      </div>
                      <span
                        style={{
              ...
              {(() => {
                const allIssues = [...failIssues, ...warnIssues, ...passIssues];
                const renderIssue = (issue: typeof allIssues[0], i: number) => {
                  const dotColor = issue.status === "fail" ? "#E74C3C" : issue.status === "warn" ? "#F0A500" : "#2ECC71";
                  const isExpanded = expandedIssue === i;
                  return (
                    <div
                      key={i}
                      className="issue-row"
                      style={{ padding: "14px 16px", borderRadius: 10, marginBottom: 4, cursor: "pointer", transition: "background 0.2s", background: isExpanded ? "rgba(26,188,156,0.04)" : "transparent" }}
                      onClick={() => setExpandedIssue(isExpanded ? null : i)}
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <div style={{ width: 10, height: 10, borderRadius: "50%", background: dotColor, flexShrink: 0 }} />
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 14, fontWeight: 600, color: "#F0EBE3" }}>{issue.title}</div>
                          <div style={{ fontSize: 12, color: "#6B7B78", marginTop: 2 }}>{issue.desc}</div>
                          {isExpanded && fixGuides[issue.title] && (
                            <div style={{ marginTop: 12, background: "#0D0F0E", borderRadius: 8, padding: 16, borderLeft: `3px solid ${dotColor}` }}>
                              <div style={{ fontSize: 12, color: "#1ABC9C", fontWeight: 600, marginBottom: 10 }}>🛠️ HOW TO FIX THIS</div>
                              {fixGuides[issue.title].map((step, j) => (
                                <div key={j} style={{ display: "flex", gap: 10, fontSize: 12, color: "#F0EBE3", marginBottom: 8, lineHeight: 1.6 }}>
                                  <span style={{ color: "#1ABC9C", fontWeight: 700, flexShrink: 0 }}>{j + 1}.</span>
                                  {step}
                                </div>
                              ))}
                            </div>
                        ...
                        {issue.status !== "pass" && (
                          <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 4, background: issue.priority === "HIGH" ? "rgba(231,76,60,0.12)" : "rgba(240,165,0,0.12)", color: issue.priority === "HIGH" ? "#E74C3C" : "#F0A500", fontFamily: "'DM Mono', monospace", flexShrink: 0 }}>
                            {issue.priority}
                          </span>
                        )}
                      </div>
                ...
                return (
                  <>
                    ...
                    {allIssues.slice(0, 1).map((issue) => renderIssue(issue, 0))}

                    {/* Remaining items + Healthgrades — blurred for non-trial users */}
                    <div style={{ position: "relative" }}>
                      <div style={!hasHealthAccess ? { filter: "blur(4px)", pointerEvents: "none", userSelect: "none" } : {}}>
                        {allIssues.slice(1).map((issue, idx) => renderIssue(issue, idx + 1))}
                        ...
                        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "14px 0", borderTop: "1px solid #1A2421" }}>
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 14, fontWeight: 600, color: data.healthgradesFound ? "#F0EBE3" : "#E74C3C" }}>
                              {data.healthgradesFound ? (data.healthgradesClaimed === false ? "⚠️ Healthgrades Profile Unclaimed" : "Found on Healthgrades ✅") : "❌ No Healthgrades Profile — Action Required"}
                            </div>
                            ...
                            <div style={{ fontSize: 12, color: "#6B7B78", marginTop: 4 }}>
                              {data.healthgradesFound ? (data.healthgradesClaimed === false ? "Unclaimed profiles lose ~15% of patient trust — anyone can edit yours and it appears abandoned. Claiming takes 5 minutes." : "A verified Healthgrades profile captures additional inquiries from the ~25% of patients who check it before booking.") : "1 in 4 patients checks Healthgrades before choosing a dentist. Without a profile, you're invisible to that entire audience."}
                            </div>
                      ...
                      {/* Overlay CTA for non-trial users */}
                      {!hasHealthAccess && (
                        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, textAlign: "center", padding: 24, background: "rgba(13,15,14,0.82)", borderRadius: 10, backdropFilter: "blur(2px)" }}>
                          <div style={{ fontSize: 14, fontWeight: 600, color: "#F0EBE3" }}>
                            Start your free 7-day trial to see your full health report
                          </div>
                          <button
                            onClick={() => setShowSaveBanner(true)}
                            style={{ padding: "9px 20px", borderRadius: 8, border: "none", background: "#1ABC9C", color: "#000", fontWeight: 700, fontSize: 13, cursor: "pointer" }}
                          >
                            Start Free Trial →
                          </button>
                        </div>
                ...
                {hasHealthAccess ? (
                  <>
                    ...
                    {[
                      ...
                      >
                        <span style={{ color: "#1ABC9C", fontWeight: 700, flexShrink: 0 }}>→</span>
                        {tip}
                      </div>
                    ))}
                  </>
                  ...
                  >
                    <div style={{ fontSize: 14, color: "#6B7B78" }}>
                      ⚡ Easy Wins unlocked with free trial — see exactly what to fix this week
                    </div>
                    <button
                      onClick={() => setShowSaveBanner(true)}
                      style={{
      ...
      {showExitIntent && (
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 14, color: "#6B7B78", marginBottom: 24 }}>
            ...
            {[
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.borderColor = "#1ABC9C";
                  e.currentTarget.style.color = "#F7F3ED";
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.borderColor = "#2A3330";
                  e.currentTarget.style.color = "#B0BDB9";
                }}
              >
                {opt}
              </button>
        ...
        >
          ...
          >
            ...
            >
              <div>
                <div
                  style={{ fontSize: 16, fontWeight: 700, color: "#F0EBE3" }}
                >
                  Send review request
                </div>
                <div style={{ fontSize: 12, color: "#6B7B78", marginTop: 3 }}>
                  Enter patient email or phone number
                </div>
              </div>
              <button
                onClick={() => setShowReviewModal(false)}
            ...
            {reviewSent ? (
              ...
              <>
                ...
                {reviewError && (
                  <div
                    style={{ fontSize: 12, color: "#E74C3C", marginBottom: 8 }}
                  >
                    {reviewError}
                  </div>
                )}
                <button
                  ...
                  style={{
                    width: "100%",
                    background: isValidContact(reviewContact)
                      ? "#1ABC9C"
                      : "rgba(26,188,156,0.2)",
                    color: isValidContact(reviewContact)
                      ? "#0D0F0E"
                      : "#4A5A57",
                    border: "none",
                    borderRadius: 9,
                    ...
                    fontSize: 14,
                    fontWeight: 700,
                    cursor: isValidContact(reviewContact)
                      ? "pointer"
                      : "default",
